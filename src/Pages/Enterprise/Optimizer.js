...
function Optimizer() {
  ...
  const [displayGatewayName, setDisplayGatewayName] = useState("");
  ...
  // console.log(displayGatewayName);
  ...
  useEffect(() => {
    ...
    if (allDelete_response) {
      console.log(allDelete_response, "----------------tghfgghffghghn-----------");
      setTrigger(true);
    }
    ...
    // Clear the interval when the component unmounts or when necessary
    return () => clearInterval(intervalId);

    // Add other dependencies as needed
  }, [token, trigger, optimizer_response, allDelete_response,]);

  async function allDelete(item) {
  ...
  useEffect(() => {
    ...
    if (optimizer_response && optimizer_response.commonEnterpriseData) {
      setOptimizerList(optimizer_response.AllEntStateLocationGatewayOptimizer);

      setDisplayEnterpriseName(
        optimizer_response.commonEnterpriseData.EnterpriseName
      );
      setDisplayStateName(optimizer_response.commonStateData.name);
      setDisplayLocationName(
        optimizer_response.commonLocationData.LocationName
      );
      setDisplayGatewayName(optimizer_response.commonGatewayData.GatewayID);
    }
          ...
          <div className="container px-6 mx-auto grid">
              ...
              {displayedData.map((item, rowIndex) => (
                  ...
                  >
                    <td className="px-4 py-3 text-sm">
                      <Link
                        to="/optimizerdetails"
                        onClick={() => handleInputChange(item, "/optimizerdetails")}
            ...
            <div className="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
              <span className="flex items-center col-span-3">
                {`Showing ${startIndex + 1}- ${optimizerList.length
                  } of ${endIndex} `}
              </span>
